...
int encodeLinearDelta8Frame(unsigned char* data, unsigned char* aFrame, unsigned char* aPrev, int pixels)
{
	...
	while (ofs < pixels)
	{
		...
		if (ofs < pixels)
		{
			...
			int l = 0;
			while (l < 255 && ofs + l + 2 < pixels && skiplength(aFrame + ofs + l, aPrev + ofs + l, 2) < 2 && runlength(aFrame + ofs + l, 2) < 2) l++;
...
int encodeLinearDelta16Frame(unsigned short* data, unsigned short* aFrame, unsigned short* aPrev, int pixels)
{
	...
	{
		// skip
		int ls = skiplength16(aFrame + ofs, aPrev + ofs, ofs + 32768 > pixels ? pixels - ofs : 32768);
		int lr = runlength16(aFrame + ofs, ofs + 32767 > pixels ? pixels - ofs : 32767);
		if (lr > ls)
		{
			data[out++] = lr;
			data[out++] = aFrame[ofs];
		...
		if (ofs < pixels)
		{
			...
			// one skip + copy segment costs at least 2 bytes, so skip until at least 2 byte run is found
			int l = 0;
			while (l < 65535 && ofs + l + 2 < pixels && skiplength16(aFrame + ofs + l, aPrev + ofs + l, 2) < 2 && runlength16(aFrame + ofs + l, 2) < 2) l++;
...
int encodeLZ1Frame(unsigned char* data, unsigned char* aFrame, unsigned char* aPrev, int pixels)
{
	...
	while (ofs < pixels)
	{
		// run
		int o = 0;
		int lz = bestLZRun1(aFrame, aPrev, ofs, pixels, o);
...
int encodeLZ2Frame(unsigned char* data, unsigned char* aFrame, unsigned char* aPrev, int pixels)
{
	...
	while (ofs < pixels)
	{
		// run
		int o = 0;
		int lz = bestLZRun2(aFrame, aPrev, ofs, pixels, o);
		int lr = runlength(aFrame + ofs, ofs + 127 > pixels ? pixels - ofs : 127);
		if (lr >= lz)
		{
			data[out++] = lr;
